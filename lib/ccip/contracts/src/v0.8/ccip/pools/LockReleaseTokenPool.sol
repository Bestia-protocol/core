...
contract LockReleaseTokenPool is TokenPool, ITypeAndVersion {
    ...
    constructor(IERC20 token, address[] memory allowlist, address armProxy, bool acceptLiquidity)
    ...
    function lockOrBurn(address originalSender, bytes calldata, uint256 amount, uint64, bytes calldata)
        external
        virtual
        override
        onlyOnRamp
    ...
    /// @dev The whenHealthy check is important to ensure that even if a ramp is compromised
    /// we're able to stop token movement via ARM.
    function releaseOrMint(bytes memory, address receiver, uint256 amount, uint64, bytes memory)
        external
        virtual
        override
        onlyOffRamp
