...
contract USDCTokenPool is TokenPool, ITypeAndVersion {
    ...
    constructor(ITokenMessenger tokenMessenger, IERC20 token, address[] memory allowlist, address armProxy)
    ...
    function releaseOrMint(bytes memory, address receiver, uint256 amount, uint64, bytes memory extraData)
        external
        override
        onlyOffRamp
    {
