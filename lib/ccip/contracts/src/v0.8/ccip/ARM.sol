...
contract ARM is IARM, OwnerIsCreator, ITypeAndVersion {
    ...
    function voteToCurse(bytes32 curseId) external {
        ...
        if (
            !curseVoteProgress.curseActive
                && curseVoteProgress.accumulatedWeight >= curseVoteProgress.curseWeightThreshold
        ) {
        ...
        for (uint256 i = 0; i < config.voters.length; ++i) {
            ...
            if (
                voter.blessVoteAddr == address(0) || voter.curseVoteAddr == address(0)
                    || voter.curseUnvoteAddr == address(0) || (voter.blessWeight == 0 && voter.curseWeight == 0)
            ) {
                return false;
            }
