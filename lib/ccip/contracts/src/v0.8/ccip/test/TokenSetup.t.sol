...
contract TokenSetup is BaseTest {
    ...
    function setUp() public virtual override {
        ...
        if (s_sourceTokens.length == 0 && s_sourcePools.length == 0) {
            ...
            s_sourcePools.push(
                address(new LockReleaseTokenPool(sourceLink, new address[](0), address(s_mockARM), true))
            );
        ...
        if (s_destTokens.length == 0 && s_destPools.length == 0) {
            ...
            s_destPools.push(
                address(new MaybeRevertingBurnMintTokenPool(destEth, new address[](0), address(s_mockARM)))
            );
