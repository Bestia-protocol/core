...
contract CustomTokenPool is TokenPool {
    ...
    function lockOrBurn(address, bytes calldata, uint256 amount, uint64, bytes calldata)
        external
        override
        whenHealthy
        onlyOnRamp
    ...
    function releaseOrMint(bytes memory, address, uint256 amount, uint64, bytes memory)
        external
        override
        whenHealthy
        onlyOffRamp
