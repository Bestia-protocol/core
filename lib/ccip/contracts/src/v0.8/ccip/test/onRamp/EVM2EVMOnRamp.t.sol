...
contract EVM2EVMOnRamp_forwardFromRouter is EVM2EVMOnRampSetup {
    ...
    function testFuzz_ForwardFromRouterSuccess(address originalSender, address receiver, uint96 feeTokenAmount)
        public
    {
        ...
        assertEq(
            expectedEvent.messageId, s_onRamp.forwardFromRouter(DEST_CHAIN_ID, message, feeTokenAmount, originalSender)
        );
    ...
    function testMessageTooLargeReverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(EVM2EVMOnRamp.MessageTooLarge.selector, MAX_DATA_SIZE, message.data.length)
        );
    ...
    function testSourceTokenDataTooLargeReverts() public {
      ...
      address(s_mockARM)
        ...
        onRamps[0] =
            TokenPool.RampUpdate({ramp: address(s_onRamp), allowed: true, rateLimiterConfig: rateLimiterConfig()});
    ...
    function testSingleTokenMessageSuccess() public {
        ...
        for (uint256 i = 0; i < feeTokenPrices.length; ++i) {
            ...
            uint32 tokenBytesOverhead =
                s_onRamp.getTokenTransferFeeConfig(message.tokenAmounts[0].token).destBytesOverhead;
    ...
    function testMessageWithDataAndTokenTransferSuccess() public {
        ...
        for (uint256 i = 0; i < feeTokenPrices.length; ++i) {
            ...
            for (uint256 j = 0; j < message.tokenAmounts.length; ++j) {
                ...
                tokenBytesOverhead +=
                    s_onRamp.getTokenTransferFeeConfig(message.tokenAmounts[j].token).destBytesOverhead;
            }
...
contract EVM2EVMOnRamp_setTokenTransferFeeConfig is EVM2EVMOnRampSetup {
    ...
    function testSetTokenTransferFeeSuccess() public {
        ...
        EVM2EVMOnRamp.TokenTransferFeeConfig memory tokenTransferFeeConfig0 =
            s_onRamp.getTokenTransferFeeConfig(address(0));
        ...
        EVM2EVMOnRamp.TokenTransferFeeConfig memory tokenTransferFeeConfig1 =
            s_onRamp.getTokenTransferFeeConfig(address(1));
