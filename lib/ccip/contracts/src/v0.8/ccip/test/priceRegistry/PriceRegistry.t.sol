    ...
    function testStaleFeeTokenReverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                PriceRegistry.StaleTokenPrice.selector, s_sourceTokens[0], TWELVE_HOURS, TWELVE_HOURS + 1
            )
        );
...
contract PriceRegistry_updatePrices is PriceRegistrySetup {
    ...
    function testOnlyGasPriceSuccess() public {
        ...
        assertEq(
            s_priceRegistry.getDestinationChainGasPrice(DEST_CHAIN_ID).value, update.gasPriceUpdates[0].usdPerUnitGas
        );
    }
...
contract PriceRegistry_getTokenAndGasPrices is PriceRegistrySetup {
    function testGetFeeTokenAndGasPricesSuccess() public {
        (uint224 feeTokenPrice, uint224 gasPrice) =
            s_priceRegistry.getTokenAndGasPrices(s_sourceFeeToken, DEST_CHAIN_ID);
    ...
    function testStaleTokenPriceReverts() public {
        ...
        Internal.GasPriceUpdate[] memory gasPriceUpdates = new Internal.GasPriceUpdate[](1);
        gasPriceUpdates[0] =
            Internal.GasPriceUpdate({destChainSelector: DEST_CHAIN_ID, usdPerUnitGas: PACKED_USD_PER_GAS});
