...
contract BurnMintSetup is BaseTest {
    ...
    function applyRampUpdates(address pool) internal {
        ...
        offRamps[0] =
            TokenPool.RampUpdate({ramp: s_burnMintOffRamp, allowed: true, rateLimiterConfig: rateLimiterConfig()});

        TokenPool.RampUpdate[] memory onRamps = new TokenPool.RampUpdate[](1);
        onRamps[0] =
            TokenPool.RampUpdate({ramp: s_burnMintOnRamp, allowed: true, rateLimiterConfig: rateLimiterConfig()});
