...
contract EVM2EVMOnRamp is IEVM2AnyOnRamp, ILinkAvailable, AggregateRateLimiter, ITypeAndVersion {
    ...
    constructor(
        ...
        if (
            staticConfig.linkToken == address(0) || staticConfig.chainSelector == 0
                || staticConfig.destChainSelector == 0 || staticConfig.defaultTxGasLimit == 0
    ...
    function forwardFromRouter(
        ...
        if (message.feeToken == i_linkToken) {
            ...
            s_nopFeesJuels += uint96(
                IPriceRegistry(s_dynamicConfig.priceRegistry).convertTokenAmount(
                    message.feeToken, feeTokenAmount, i_linkToken
                )
            );
        }
    ...
    function applyPoolUpdates(Internal.PoolUpdate[] memory removes, Internal.PoolUpdate[] memory adds)
        external
        onlyOwner
    {
